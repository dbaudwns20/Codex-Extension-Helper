...
function mentionsFromDrop(uriListText, resourceUrlsText, rootCandidates) {
  ...
  for (const raw of uriListText.split(/\r?\n/u)) {
    ...
    try {
      ...
      if (!filePaths.has(pathKey)) target += '/';
      const label = normalizedPath.split('/').at(-1) || normalizedPath;
      mentions.push({ label, path: target, fsPath });
    } catch {
...
export function unpatchBundleSourceVersion(source, version) {
  const startMarker = `/* codex-explorer-drop-chips:start:v${version} */`;
  const endMarker = `/* codex-explorer-drop-chips:end:v${version} */`;
  ...
  if (end === endMarker.length - 1) throw new Error('Malformed Codex drop patch markers');
...
export function unpatchBundleSource(source) {
  return unpatchBundleSourceVersion(source, PATCH_VERSION);
}
